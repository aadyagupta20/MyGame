...
fun place-jerry(w :: World):
  ...
  if w.is-start-screen:
    ...
    # Place Jerry
    jerry-scene = I.place-image(jerry, w.pos.x, HEIGHT - 27, scene-with-score)
    ...
    scene-with-items = fold(place-single-item, jerry-scene, w.items)
...
fun update-world(w :: World):
  ...
  if w.is-start-screen:
    ...
    if not(is-empty(colliding-bombs)):
      world(w.pos, w.items, w.score, true, false)
    else:
...
fun handle-mouse(w :: World, x :: Number, y :: Number, event :: String):
  ...
  if w.is-start-screen and (event == "button-down"):
    if (x >= ((WIDTH / 2) - 100)) and (x <= ((WIDTH / 2) + 100)) and
      ...
      world(w.pos, empty, 0, false, false)  # Start the game
    else:
      w
    end
  else:
    ...
    ask:
      | key == "left" then:
        if (w.pos.x - KEY_DISTANCE) > 0:
          world(posn(w.pos.x - KEY_DISTANCE, w.pos.y), w.items, w.score, false, false)
        else:
          w
        end
      | key == "right" then:
